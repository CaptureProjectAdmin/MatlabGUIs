...
for k=1:length(RWNAppFiles)
    ...
    %
    % ntp_imu = []; d_imu = []; fs_imu = [];
    % if (isempty(Files.drift_csv_file)||isempty(Files.pupil_imu_file))
    %     fprintf('Pupil IMU or drift files are missing for %s %s\n',ptname,walkname);
    % else
    %     [ntp_imu,d_imu,fs_imu] = findPupilIMUNTP(Files);
    %     if isempty(ntp_imu)
    %         fprintf('Pupil IMU data is empty for %s %s\n',ptname,walkname);
    %     end
    % end
    % save(rwnappfile,'ntp_imu','d_imu','fs_imu','-append');
