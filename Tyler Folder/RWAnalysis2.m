    ...
    methods %initialization, calculations, etc
            ...
            for k=1:TotalWalks
                ...
                if ~ismember(WalkNum,obj.DisabledWalks{obj.PatientIdx})
                    ...
                    if isfile(Fname)
                        ...
                        %Fixing for misspelled descriptions
                        SS.evnts_tbl.Description = regexprep(SS.evnts_tbl.Description,'\[duringnWalk\]','[duringWalk]');
                        SS.evnts_tbl.Description = regexprep(SS.evnts_tbl.Description,'\[duringwalk\]','[duringWalk]');
                        SS.evnts_tbl.Description = regexprep(SS.evnts_tbl.Description,'\[les\]','[less]');
                        SS.evnts_tbl.Description = regexprep(SS.evnts_tbl.Description,'\[parital\]','[partial]');
                        SS.evnts_tbl.Description = regexprep(SS.evnts_tbl.Description,'\[patial\]','[partial]');
                        ...
                        if isempty(obj.MultTable)
                            ...
                            flabels = {'peak_freq','peak_height','peak_width','aperiodic_offset','aperiodic_exponent','fit_rsquared','fit_error','theta_gamma'};
                            ...
                            parfor m=1:nseg %by segment
                                ...
                                fidx_theta = f>4 & f<=8;
                                fidx_gamma = (f>30 & f<=57.5) | (f>65 & f<=85); %avoid 60 and 62.5 with 2.5 border
                                theta_gamma_ratio = mean(S(:,fidx_theta),2)./mean(S(:,fidx_gamma),2); %4x1
                                for n=1:4 %by chan
                                    if ~any(nan_idx(T(m,:),n))
                                        fr = fooof(f,S(n,:),cparams.fpass,fparams,1); %fr.peak_params = [freq, height (aperiodic removed), width] (85Hz matches upper lim in plotMultTransSpecGramPerm)
                                        ...
                                        switch fparams.aperiodic_mode
                                            case 'fixed'
                                                FR(m,:,n) = [pkp,fr.aperiodic_params,fr.r_squared,fr.error,theta_gamma_ratio(n)];
                                            case 'knee'
                                                FR(m,:,n) = [pkp,fr.aperiodic_params([1,3]),fr.r_squared,fr.error,theta_gamma_ratio(n)];
                                        end
                                    end
                                end
                            end
        ...
        function getMultData(obj,varargin)
            ...
            for k=1:length(obj.PatientList)
                ...
                ds_flabels = {'peak_freq','peak_height','peak_width','aperiodic_offset','aperiodic_exponent','fit_rsquared','fit_error','theta_gamma'};
                ...
                for m=1:4 %by channel
                    ...
                    fidx_theta = ds_f_spec_mt>4 & ds_f_spec_mt<=8;
                    fidx_gamma = (ds_f_spec_mt>30 & ds_f_spec_mt<=57.5) | (ds_f_spec_mt>65 & ds_f_spec_mt<=85); %avoid 60 and 62.5 with 2.5 border
                    theta_gamma_ratio = mean(ds_S(:,fidx_theta),2)./mean(ds_S(:,fidx_gamma),2); %nsegx1

                    %fooof
                    ds_FR = nan(ds_nseg,length(ds_flabels));
                    parfor n=1:ds_nseg
                        ds_fr = fooof(ds_f_spec_mt, ds_S(n,:), [2,85], ds_settings, 1); %fr.peak_params = [freq, height (aperiodic removed), width] (85Hz matches upper lim in plotMultTransSpecGramPerm)
                        ...
                        switch ds_settings.aperiodic_mode
                            case 'fixed'
                                ds_FR(n,:) = [ds_pkp,ds_fr.aperiodic_params,ds_fr.r_squared,ds_fr.error,theta_gamma_ratio(n)];
                            case 'knee'
                                ds_FR(n,:) = [ds_pkp,ds_fr.aperiodic_params([1,3]),ds_fr.r_squared,ds_fr.error,theta_gamma_ratio(n)];
                        end
                    end
        ...
        function getFilteredMultTransData(obj,transrng,varargin)
            ...
            nflds_fo = size(flds_fo,2); %7 fields of data (8 now - added theta_gamma_ratio)
            ...
            for k=1:size(uPtWkCh,1)
                ...
                %fooof exponent and theta gamma
        ...
        function calcMultFooofStats(obj,varargin)
            %Runs fooof for the full walk for all patients and compares
            %knee to fixed
            if isempty(obj.MultTable)
            ...
            D = {};
            for k=1:5
                D = cat(1,D,obj.MultTable{k}.d_np);
            end
            ...
            FR_fooofedspec = [];
            FR_apfit = [];
            for q=1:length(D) %by patient/walk
                disp(q/length(D));
                ...
                T(rm_idx,:) = [];
                nseg = size(T,1); %number of windows
                ntrials = size(d,2); %actually chans
                
                [~,f] = mtspectrumc(d(T(1,:),:),cparams);
                freqs = f(f<=fpass(2)); %same as fr.freqs in fooof output
                fcnt = length(freqs);
                ...
                for k=1:2 %fixed, knee
                    ...
                    parfor m=1:nseg %by segment
                        ...
                        for n=1:ntrials %by trial
                            if ~any(nan_idx(T(m,:),n))
                                fr = fooof(f,S(n,:),fpass,fparams,1); %fr.peak_params = [freq, height (aperiodic removed), width] (85Hz matches upper lim in plotMultTransSpecGramPerm)
                                if isempty(fr.peak_params)
                                    pkp = nan(1,3);
                                else
                                    [~,midx] = max(fr.peak_params(:,2)); %find the highest peak
                ...
                FR = cat(1,FR,Fr);
                FR_powerspec = cat(1,FR_powerspec,Fr_powerspec);
                FR_fooofedspec = cat(1,FR_fooofedspec,Fr_fooofedspec);
                FR_apfit = cat(1,FR_apfit,Fr_apfit);
            end %by patient/walk          
            ...
            test_band = [4,50];
            fidx = freqs>=test_band(1) & freqs<test_band(2);
    ...
    methods %general plotting
        ...
        function varargout = plotMultTransSpecGramPerm(obj,varargin)
            ...
            if plottrials
                ...
                set(aH2,'FontName','Arial','FontSize',18,'TitleFontSizeMultiplier',0.8);
            ...
            if plotfooofcomp
                ...
                set(aH4,'FontName','Arial','FontSize',18,'TitleFontSizeMultiplier',0.6);
